...
from vars import AYRIK_THEMES, OOMOX_TEMPLATES, OOMOX_COLORS, BASE_CONFIGS, HOME, SESSION_TYPE
...
class CopyOption(BaseOption):
    ...
    def _run(self, theme_name: str) -> None:
        cfg_path = AYRIK_THEMES / theme_name / self.name

        if not self.path_to.parent.exists():
            logging.error(
                f"Theme \"{theme_name}\" has not been applied to \"{
    self._id}\"! "
        ...
        logging.error(
            ...
            f"There is no {
    'folder' if self.is_dir else 'file'} \"{
        self.name}\" in the theme folder"
        )
...
class CopyOrGenOption(BaseOption):
    ...
    self._id}\"! "
        ...
        logging.error(
            f"Theme \"{theme_name}\" has not been applied to \"{self._id}\"!"
            f"There is no \"{
    self.name}\" file in the theme folder. No generation option."
        )
...
class TmuxCfgOption(BaseOption):
    ...
    def _run(self, theme_name: str) -> None:
        ...
        custom_prefs: Path = AYRIK_THEMES / theme_name / self.name
    ...
    self._id}\"! "
        ...
        try:
            if subprocess.run(["pgrep", "tmux"],
                              stdout=subprocess.PIPE).stdout.decode().strip():
                subprocess.run(["tmux", "source", str(tmux)], check=True)
        except Exception:
            logging.warning(
                "Failed to update the theme for tmux in an open session.")
...
class FishOption(BaseOption):
    ...
    self._id}\"! "
        ...
        if cfg_path.exists() and cfg_path.is_file():
            ...
            if self.apply_theme:
                subprocess.Popen(
                    ['fish', '-c', 'echo "y" | fish_config theme save ayrik'],
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
                    text=True
                )
...
class WaybarCfgOption(BaseOption):
    ...
    def _run(self, theme_name: str) -> None:
        ...
        if cfg_path.exists() and cfg_path.is_file():
            ...
            if self.reload:
                try:
                    wb_exists = subprocess.run(
                        ['pgrep', '-x', 'waybar'], capture_output=True, text=True).stdout
                    if wb_exists:
                        subprocess.run(
                            ['pkill', '-SIGUSR2', 'waybar'], check=True)
                except Exception:
...
class GTKOption(BaseOption):
    ...
    def generate_gtk_2_3(self, path_to_theme: Path,
                         oomox_colors_path: str, theme_name: str) -> bool:
        try:
            subprocess.check_output(["which", "oomox-cli"])
        except subprocess.CalledProcessError:
            logging.error(
                "GTK theme is not installed. A dependency is required - \"oomox\"")
            return False

        try:
            subprocess.run(
                ["oomox-cli", oomox_colors_path, "-o",
                    theme_name, "-m", "all", "-d", "true"],
            ...
            logging.error(
                f"GTK theme is not installed. Failed to generate gtk 2/3 theme: {traceback.format_exc()}")
            return False

        return True

    def generate_gtk_4(self, path_to_theme: Path,
                       gtk4_template: Path, oomox_colors_path: Path) -> bool:
        if not (OOMOX_TEMPLATES / gtk4_template).exists():
            logging.error(
    f"GTK4 theme is not installed. Template file not found: {gtk4_template}")
            return False

        gtk3_path: Path = path_to_theme / "gtk-3.0"
        gtk4_path: Path = path_to_theme / "gtk-4.0"

        generated_gtk4 = generate_theme(
    template_name=self.gtk4_template_name,
     oomox_colors=oomox_colors_path)
        if generated_gtk4 is not None:
            ...
            logging.warning(
                "The GTK4 theme is not installed due to an unknown error!")
            return False
    ...
    def apply_gtk_themes(self, gtk_configs: List[Path], theme_name: str):
        for gtk_cfg in gtk_configs:
            if not gtk_cfg.parent.exists():
                logging.warning(
    f"The theme cannot be applied to the \"{
        gtk_cfg.name}\" file, because the path \"{
            str(gtk_cfg)}\" does not exist")
    ...
    f"Net/ThemeName \"{theme_name}\"",
     content)
                ...
                try:
                    subprocess.run(
                        ["killall", "-HUP", "xsettingsd"], check=True)
                except Exception:
                    logging.warning("Failed to set theme with xsettingsd")

    def _run(self, theme_name: str) -> None:
        oomox_colors_path: Path = OOMOX_COLORS(theme_name)
        theme_name: str = f"ayrik-{theme_name}"
        path_to_theme: Path = HOME / ".themes" / theme_name

        if not oomox_colors_path.exists():
            logging.error(
    f"GTK theme is not installed. There is no \"{oomox_colors_path}\" file.")
            return

        if not path_to_theme.exists():
            gtk23 = self.generate_gtk_2_3(
    path_to_theme, str(oomox_colors_path), theme_name)
            if not gtk23:
                return

        if not (path_to_theme / "gtk-4.0").exists():
            gtk4 = self.generate_gtk_4(
    path_to_theme,
    self.gtk4_template_name,
     oomox_colors_path)
