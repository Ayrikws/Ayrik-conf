...
from vars import (
    SESSION_TYPE, AYRIK_DIR, AYRIK_CONFIG,
    WALLPAPER_SYMLINC, AYRIK_ASSETS
)


class Config:
    __slots__ = ()
    symlink_wallpapers = Path.home() / ".config" / "ayrik" / "current_wallpaper"
    ...
    def __load_yaml(cls) -> dict:
        ...
        """
        if not AYRIK_CONFIG.exists():
            raise NoConfigFile()

        with open(AYRIK_CONFIG, 'r') as f:
    ...
    def _validate_theme(theme_name: str, wallpapers: List[str]) -> Optional[Theme]:
        ...
        path_to_theme: Path = AYRIK_DIR / "themes" / theme_name
        icon = AYRIK_ASSETS / "default-theme-icon.png"
